...
export default function MunicipalityList({
  ...
  const items = useMemo(
    () =>
      municipalities
        .filter((m) => visibleCategories.has(categoryOf(m)))
  ...
  return (
    ...
    >
      <p className="mb-2 uppercase tracking-wide text-sub">Gemeenten ({items.length})</p>
      <ul className="min-h-0 flex-1 space-y-0.5 overflow-y-auto pr-1 text-sub">
        {items.map((m) => {
        ...
        {items.length === 0 && <li className="px-1 py-1">Geen gemeenten</li>}
      </ul>
    </div>
  );
}
